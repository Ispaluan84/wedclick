import { useState, useEffect } from 'react'
import { motion }              from 'framer-motion'
import { ArrowDown }           from 'lucide-react'
import noviosHero              from '../assets/images/novios-hero.jpg'

const fadeUp = (delay = 0) => ({
  initial:    { opacity: 0, y: 30 },
  animate:    { opacity: 1, y: 0  },
  transition: { duration: 0.8, delay, ease: 'easeOut' },
})

function Hero({ config }) {
  const nombre1    = config?.novio1        || 'Elena'
  const nombre2    = config?.novio2        || 'Marcos'
  const fecha      = config?.fecha         || '2026-06-14T12:00:00'
  const lugar      = config?.lugar         || 'Sevilla, España'
  const opacity    = config?.hero?.overlayOpacity ?? 0.5

  const [timeLeft, setTimeLeft] = useState(getTimeLeft(fecha))

  useEffect(() => {
    const timer = setInterval(() => setTimeLeft(getTimeLeft(fecha)), 1000)
    return () => clearInterval(timer)
  }, [fecha])

  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden">

      <div className="absolute inset-0">
        <img
          src={config?.fotoHero || noviosHero}
          alt={`${nombre1} & ${nombre2}`}
          className="w-full h-full object-cover object-center"
        />
        <div className="absolute inset-0 bg-verde-oscuro" style={{ opacity }} />
        <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-black/40 to-transparent" />
      </div>

      <div className="relative z-10 text-center px-6 flex flex-col items-center gap-6">

        <motion.div {...fadeUp(0.2)} className="flex items-center gap-3">
          <div className="w-12 h-px bg-tierra opacity-70" />
          <span className="text-tierra text-lg">✦</span>
          <div className="w-12 h-px bg-tierra opacity-70" />
        </motion.div>

        <motion.h1
          {...fadeUp(0.3)}
          className="font-serif text-6xl md:text-8xl text-white leading-tight"
        >
          {nombre1}
          <span className="block text-tierra text-4xl md:text-5xl font-light italic my-2">&</span>
          {nombre2}
        </motion.h1>

        <motion.div {...fadeUp(0.4)} className="flex flex-col items-center gap-2">
          <p className="font-sans text-white/80 tracking-widest uppercase text-sm">
            {new Date(fecha).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
          <p className="font-sans text-white/60 text-xs tracking-widest uppercase">
            {lugar}
          </p>
        </motion.div>

        <motion.div {...fadeUp(0.5)} className="flex items-center gap-3">
          <div className="w-12 h-px bg-tierra opacity-70" />
          <span className="text-tierra text-lg">◆</span>
          <div className="w-12 h-px bg-tierra opacity-70" />
        </motion.div>

        <motion.div {...fadeUp(0.6)} className="flex items-center gap-6 md:gap-10">
          {[
            { label: 'Días',     value: timeLeft.dias     },
            { label: 'Horas',    value: timeLeft.horas    },
            { label: 'Minutos',  value: timeLeft.minutos  },
            { label: 'Segundos', value: timeLeft.segundos },
          ].map((item) => (
            <div key={item.label} className="flex flex-col items-center gap-1">
              <span className="font-serif text-4xl md:text-5xl text-white font-bold">
                {String(item.value).padStart(2, '0')}
              </span>
              <span className="font-sans text-xs text-white/60 tracking-widest uppercase">
                {item.label}
              </span>
            </div>
          ))}
        </motion.div>

        <motion.a {...fadeUp(0.7)} href="#confirmacion" className="btn-natural mt-4">
          Confirmar asistencia
        </motion.a>

      </div>

      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1, delay: 1.2 }}
        className="absolute bottom-8 left-1/2 -translate-x-1/2 flex-col items-center gap-2 hidden md:flex"
      >
        <span className="font-sans text-xs text-white/60 tracking-widest uppercase">Descubrir</span>
        <motion.div
          animate={{ y: [0, 6, 0] }}
          transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
        >
          <ArrowDown size={16} className="text-white opacity-60" />
        </motion.div>
      </motion.div>

    </section>
  )
}

function getTimeLeft(fecha) {
  const wedding = new Date(fecha || '2026-06-14T12:00:00')
  const today   = new Date()
  const diff    = Math.max(0, wedding - today)
  return {
    dias:     Math.floor(diff / (1000 * 60 * 60 * 24)),
    horas:    Math.floor((diff / (1000 * 60 * 60)) % 24),
    minutos:  Math.floor((diff / (1000 * 60)) % 60),
    segundos: Math.floor((diff / 1000) % 60),
  }
}

export default Hero